            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-4 h-4 rounded border-midnight-600 bg-midnight-800 text-camp-500"
                />
              ...
              <div className="flex gap-3">
                ...
                <button type="button" onClick={() => setShowBuilder(false)} className="btn-secondary">
                  Cancel
                </button>
              </div>
            </form>
              ...
              <div key={link.id} className="gradient-border p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-midnight-400 text-sm truncate mb-2">{buildFullUrl(link)}</p>
                    
                    <div className="flex flex-wrap gap-2">
                      {link.utm_source && (
                        <span className="px-2 py-1 bg-midnight-800 text-xs rounded">source: {link.utm_source}</span>
                      )}
                      {link.utm_medium && (
                        <span className="px-2 py-1 bg-midnight-800 text-xs rounded">medium: {link.utm_medium}</span>
                      )}
                      {link.utm_campaign && (
                        <span className="px-2 py-1 bg-midnight-800 text-xs rounded">campaign: {link.utm_campaign}</span>
                      )}
                    </div>
                  </div>
